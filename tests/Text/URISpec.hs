...
spec = do
  ...
  describe "parse" $ do
    ...
    context "when URI has scheme" $
      it "accepts colon in path components" $
        parse urip "" `shouldSucceedOn` "https:/fir:st/se:cond"
    context "when URI is a network-path reference" $
      it "accepts colon in path components" $
        parse urip "" `shouldSucceedOn` "//host/fir:st/se:cond"
    context "when URI is a relative-path reference" $
      it "interprets colon in the first path segment as scheme delimiter" $ do
        scheme <- URI.mkScheme "fir"
        ppst <- URI.mkPathPiece "st"
        ppsecond <- URI.mkPathPiece "se:cond"
        parse urip "" "fir:st/se:cond" `shouldParse` URI
          { uriScheme = Just scheme
          , uriAuthority = Left False
          , uriPath = Just (False, ppst :| [ppsecond])
          , uriQuery = []
          , uriFragment = Nothing
          }
...
testURI = "https://mark%3a%40:secret:%40@github.com:443/mrkkrp/modern-uri+:@?foo:@=bar+:@#fragment:@"
